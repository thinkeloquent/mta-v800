...
describe('Fastify Integration', () => {
     ...
     */
    async function createTestServer(envPath?: string): Promise<FastifyInstance> {
        ...
        // Add secret endpoint
        fastify.get<{ Params: { key: string } }>('/secret/:key', async (request) => {
        ...
        fastify.get<{ Params: { key: string } }>('/secret-value/:key', async (request) => ({
            key: request.params.key,
    ...
    describe('Server Decorators', () => {
        it('should decorate server with vault loaded count', async () => {
            ...
            expect((server as any).vaultLoaded).toBeGreaterThan(0);
        });
    });
