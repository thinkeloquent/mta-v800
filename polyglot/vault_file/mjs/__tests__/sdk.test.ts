...
describe('SDK Module', () => {
    ...
    beforeEach(() => {
        ...
        (EnvStore as any).instance = undefined;
    });
    ...
    describe('Error Handling', () => {
        it('should handle loadConfig exception gracefully', async () => {
            ...
            (EnvStore as any).onStartup = async () => {
            ...
            try {
                ...
                (EnvStore as any).onStartup = originalOnStartup;
            }
        });
    });
    ...
    describe('VaultFileSDKBuilder', () => {
        describe('Statement Coverage', () => {
            it('should set custom env path', () => {
                ...
                expect((sdk as any).envPath).toBe('/custom/.env');
            });

            it('should set base64 parsers', () => {
                const parsers = { json: (x: string) => JSON.parse(x) };
                const sdk = VaultFileSDK.create()
                    .withBase64Parsers(parsers)
                    .build();

                expect((sdk as any).base64Parsers).toHaveProperty('json');
            });
        });
        ...
        describe('Builder Chaining', () => {
            it('should support method chaining', () => {
                const sdk = VaultFileSDK.create()
                    .withEnvPath('/custom/.env')
                    .withBase64Parsers({ test: (x: string) => x })
                    .build();

                expect((sdk as any).envPath).toBe('/custom/.env');
                expect((sdk as any).base64Parsers).toHaveProperty('test');
            });
        });
    });
});
